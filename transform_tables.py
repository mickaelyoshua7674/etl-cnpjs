...
clean_chars_fn = lambda values: values.apply(lambda x: re.sub(r"['%\(\):]", "", x))
...
# empresas = pd.read_csv(empresas_path, chunksize=100_000, dtype={"cnpj_basico": "str"}, low_memory=False)
...
estabelecimentos = pd.read_csv(estabelecimentos_path, chunksize=100_000, dtype={"cnpj_basico": "str",
                                                                                ...
                                                                                "data_situacao_especial": "str"}, low_memory=False)

header = True
columns_to_clean_estab = ["nome_fantasia", "logradouro", "complemento", "bairro", "numero", "email", "nome_cidade_exterior"]
...
for chunk in estabelecimentos:
    ...
    chunk = chunk.fillna(value={"identificador": 0,
                                ...
                                "email": "Nao informado",
                                "nome_cidade_exterior": "Nao informado"})
    ...
    chunk[columns_to_clean_estab] = chunk[columns_to_clean_estab].apply(clean_chars_fn)
...
# cnae = pd.read_csv(cnae_path, low_memory=False)
...
# motivo = pd.read_csv(motivo_path, low_memory=False)
...
# natjur = pd.read_csv(natjur_path, low_memory=False)
...
# qualificacoes = pd.read_csv(qualificacoes_path, low_memory=False)
...
# municipios = pd.read_csv(municipios_path, low_memory=False)
...
# paises = pd.read_csv(paises_path, low_memory=False)
...
# simples = pd.read_csv(simples_path, chunksize=100_000, low_memory=False)
...
# socios = pd.read_csv(socios_path, chunksize=100_000, low_memory=False)
